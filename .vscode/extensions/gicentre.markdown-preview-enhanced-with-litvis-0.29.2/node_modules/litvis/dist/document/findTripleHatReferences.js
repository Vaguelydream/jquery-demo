"use strict";
// inspired by https://github.com/zestedesavoir/zmarkdown/blob/master/packages/remark-kbd/src/index.js
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.findTripleHatReferences = void 0;
const is_whitespace_character_1 = __importDefault(require("is-whitespace-character"));
const HAT = "^";
const TRIPLE_HAT = "^^^";
const locator = (value, fromIndex) => {
    const index = value.indexOf(TRIPLE_HAT, fromIndex);
    return index;
};
// Usage of "this" requires suppressing func-style eslint rule
// eslint-disable-next-line func-style
function findTripleHatReferences() {
    // eslint-disable-next-line func-style
    function inlineTokenizer(eat, value, silent) {
        if (!this.options.gfm ||
            value.charAt(0) !== HAT ||
            value.charAt(1) !== HAT ||
            value.charAt(2) !== HAT ||
            value.startsWith(HAT.repeat(6)) ||
            is_whitespace_character_1.default(value.charAt(3))) {
            return;
        }
        let character = "";
        let previous = "";
        let preceding = "";
        let prePreceding = "";
        let subvalue = "";
        let index = 2;
        const length = value.length;
        const now = eat.now();
        now.column += 3;
        now.offset += 3;
        while (++index < length) {
            character = value.charAt(index);
            if (character === HAT &&
                previous === HAT &&
                preceding === HAT &&
                (!prePreceding || !is_whitespace_character_1.default(prePreceding))) {
                /* istanbul ignore if - never used (yet) */
                if (silent) {
                    return true;
                }
                return eat(TRIPLE_HAT + subvalue + TRIPLE_HAT)({
                    type: "tripleHatReference",
                    data: {
                        info: subvalue,
                    },
                });
            }
            subvalue += preceding;
            prePreceding = preceding;
            preceding = previous;
            previous = character;
        }
    }
    inlineTokenizer.locator = locator;
    const Parser = this.Parser;
    // Inject inlineTokenizer
    const inlineTokenizers = Parser.prototype.inlineTokenizers;
    const inlineMethods = Parser.prototype.inlineMethods;
    inlineTokenizers.tripleHatReference = inlineTokenizer;
    inlineMethods.splice(inlineMethods.indexOf("text"), 0, "tripleHatReference");
    const Compiler = this.Compiler;
    // Stringify
    if (Compiler) {
        const visitors = Compiler.prototype.visitors;
        visitors.tripleHatReference = function (node) {
            return `${TRIPLE_HAT}${this.all(node).join("")}${TRIPLE_HAT}`;
        };
    }
}
exports.findTripleHatReferences = findTripleHatReferences;
//# sourceMappingURL=findTripleHatReferences.js.map